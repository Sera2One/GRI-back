...
export async function getMessage(req, res) {
  ...
  if (destinationCode) {
    await models.Destination.findAndCountAll({
				...
				[Op.and]: [
					{
						[Op.or]: [
							{
								destSenderCode: destinationCode, // camelCase comme défini dans le modèle
								usrCode: senderCode, // camelCase
							},
							{
								destSenderCode: senderCode, // camelCase
								usrCode: destinationCode, // camelCase
							},
						],
					},
					{
						destIsDeleted: destIsDeleted, // camelCase
					},
				],
			},
			include: [
				{
					model: models.Users,
					as: 'usrCodeUser', // doit correspondre à l'alias défini dans l'association
					attributes: [
						'usr_code',
						'usr_name',
						'usr_firstname',
				...
				},
				{
					...
					as: 'msgCodeMessage', // doit correspondre à l'alias défini dans l'association
					attributes: [
						'msg_code',
						'usr_code',
						'msg_contenu',
			...
			limit: limit,
			offset: offset,
			order: [['dest_created_date', 'DESC']], // ici utiliser le nom de la colonne réelle
			attributes: [
				'msg_code',
				'msg_code',
				['usr_code', 'dest_code'],

...
const msgFilename = "msg_" + moment().format("YYYY-MM-DD_HH-mm-ss");
...
export const messageIO = (socket, io) => {
	socket.on('user-new-message', async data => {
	...
	socket.on(
		...
		async ({ page = 1, limit = 200, isDeleted = false }, callback) => {
			...
			try {
				...
				//console.log('rows aa', rows.get({ plain: true }));
				//console.log('rows aa', unreadCounts);
...
export const postMessage = async (ioData, socket, io) => {
	...
	try {
		...
		try {
			if (isValidePieceJoint) {
				// Save to data base
				const saveStatus = await models.MsgFileJoint.bulkCreate(fileMetadata);

...
class MessageHandler {
	...
	async handleForwardMessage(data, callback) {
		try {
			const { groupId, sourceCode, destinationType, destinationListCode } =
				data;
			...
			const newMsgData = await forwardMessage({
				sourceCode,
			...
			await this.emitForwardedMessage(newMsgData, destinationListCode); 
	...
	async emitForwardedMessage(newMsgData, destinationListCode) {
		const connectedUsers = await this.getConnectedUsers();

		destinationListCode.forEach(dest => {
			const userOnline = connectedUsers.find(u => u.usrCode === dest.destCode);
			if (userOnline) {
				// Notification nouveau message
				this.io.to(userOnline.usrCode).emit('user-new-message', {
					msgCode: newMsgData.id,
					usrCode: dest.destCode,
					destSenderCode: this.socket.handshake.auth.usrCode,
					destIsReaded: false,
					destIsDeleted: false,
					msgContenu: newMsgData.content,
					msgCreatedDate: newMsgData.createdAt,
					msgParentCode: null,
					msgPieceJoint: JSON.stringify(newMsgData.files),
					senderImg: this.socket.handshake.auth.usrImg,
					senderName: this.socket.handshake.auth.usrName,
					senderFirstname: this.socket.handshake.auth.usrFirstname,
				});

...
class MessageHandler {
	...
	initializeHandlers() {
		...
		this.socket.on(
			...
			this.handleForwardMessage.bind(this),
		);
		this.socket.on(
			'user-forward-message',
			this.handleForwardMessageUnified.bind(this),
		);
	}
	...
	async handleForwardMessage(data, callback) {
		try {
			const { sourceType, sourceId, destinationType, destinationId } = data;

			// Valider que l'utilisateur a accès au message source
			await validateForwardAccess(
				sourceType,
				sourceId,
				this.socket.userID,
			);
			...
			const result = await forwardMessage({
				sourceType,
				sourceId,
				forwarderUsrCode: this.socket.userID,
				destinationType,
				destinationId,
			});

			// Émettre le nouveau message
			await this.broadcastForwardedMessage(
				result.newId,
				destinationType,
				destinationId,
			);

			callback?.({ success: true, newId: result.newId });
		} catch (error) {
	...
	prepareMessageData(ioData, msgCode, msgCreatedDate) {
		const { msgContenu, usrCode, msgParentCode } = ioData;
		const messageData = {
			msgCode: msgCode,
			usrCode: usrCode,
	...
	async forwardMessage(originalMsgCode, forwarderUsrCode, destCodeListe) {
		const transaction = await sequelize.transaction();

		try {
			// 1. Récupérer le message source + fichiers
			const originalMessage = await models.Messages.findByPk(originalMsgCode, {
				include: [
					{
						model: models.MsgFileJoint,
						as: 'msgFileJoints',
			...
			// 2. Générer un nouveau msgCode
			const newMsgCode = await this.generateMessageCode();
			const msgCreatedDate = getPreciseGMTTime().iso;

			// 3. Préparer le nouveau message
			const newMessageData = {
				msgCode: newMsgCode,
				usrCode: forwarderUsrCode,
				msgContenu: originalMessage.msg_contenu,
				msgCreatedDate: msgCreatedDate,
				msgModifDate: null,
				msgParentCode: originalMessage.msg_parent_code,
				msgHasPieceJoint: originalMessage.msg_has_piece_joint,
				// 🔥 Nouveaux champs de traçabilité
				msgForwardedFrom: originalMsgCode, // référence au message source
				msgForwardedFromUser: originalMessage.usrCode, // qui a écrit l’original
			};

			// 4. Préparer les destinations
			const newDestinationData = destCodeListe.map(dest => ({
				msgCode: newMsgCode,
				usrCode: dest.destCode,
			...
			// 5. Préparer les fichiers joints (copie des métadonnées)
			let newFileMetadata = [];
			if (originalMessage.msgFileJoints?.length > 0) {
				newFileMetadata = originalMessage.msgFileJoints.map((file, index) => ({
					msgFileCode: `${newMsgCode}-${index}`, // nouveau code
					usrCode: forwarderUsrCode, // propriétaire du transfert
					msgCode: newMsgCode,
					msgFilePath: file.msg_file_path, // ⚠️ même chemin physique !
					msgFileType: file.msg_file_type,
					msgFileExtension: file.msg_file_extension,
					msgFileClientFileName: file.msg_file_client_file_name,
					msgFileIsDeleted: false,
					msgFileCreatedDate: msgCreatedDate,
					msgFileSize: file.msg_file_size,
				}));
			}

			// 6. Sauvegarder en base
			await models.Messages.create(newMessageData, { transaction });
			await models.Destination.bulkCreate(newDestinationData, { transaction });
			if (newFileMetadata.length > 0) {
				await models.MsgFileJoint.bulkCreate(newFileMetadata, { transaction });
			}

			await transaction.commit();

			// 7. Préparer les données pour l’émission
			return {
				newMsgCode,
				newMessageData,
				newDestinationData,
	...
	async emitForwardedMessage(newMsgData, destCodeListe, forwarderUsrInfo) {
		const connectedUsers = await this.getConnectedUsers();

		destCodeListe.forEach(dest => {
			const userOnline = connectedUsers.find(u => u.usrCode === dest.destCode);
			if (userOnline) {
				this.io.to(userOnline.usrCode).emit('user-new-message', {
					msgCode: newMsgData.newMsgCode,
					usrCode: dest.destCode,
					destSenderCode: forwarderUsrInfo.usrCode,
					destIsReaded: false,
					msgContenu: newMsgData.newMessageData.msgContenu,
					msgCreatedDate: newMsgData.newMessageData.msgCreatedDate,
					msgParentCode: newMsgData.newMessageData.msgParentCode,
					msgPieceJoint: JSON.stringify(newMsgData.newFileMetadata),
					senderImg: forwarderUsrInfo.usrImg,
					senderName: forwarderUsrInfo.usrName,
					senderFirstname: forwarderUsrInfo.usrFirstname,
					// ajoute ce que ton frontend attend
				});
			}
		});
	}

	async deleteOrphanedFile(msgFileCode) {
		const transaction = await sequelize.transaction();
		try {
